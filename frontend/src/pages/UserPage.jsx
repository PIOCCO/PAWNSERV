...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (!data.username) throw new Error("Username not found in response");
      ...
      <div className="profile-header">
        ...
        <div className="profile-info">
          ...
          <div className="button-group">
            ...
            <button className="myservices-btn" onClick={() => navigate("/myservices")}>
              My Services
            </button>
          </div>
        </div>
  ...
  <img key={index} src={`/images/post-${index + 1}.jpg`} alt="User post" className="post" />
))}
      </div>
    </div>
  );
